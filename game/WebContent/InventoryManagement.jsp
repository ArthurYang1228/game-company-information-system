<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="models.Connector, models.Inventory, models.InventoryDAO, java.util.ArrayList"%>
...
<body>
	...
	<%
		...
		if(deletedId != null) {
			inventoryDAO.delete(inventoryDAO.getInventory(Integer.parseInt(deletedId)));
		}
	%>
	...
	<% 
		...
		if(name != null && quantityInput != null && safetyStockInput != null) {
			inventoryDAO.insert(new Inventory(name, quantity, safetyStock, state, price ,orderTime));
			out.println("新增員工成功");
	...
	<% 
		ArrayList<Inventory> inventorys = inventoryDAO.getAllInventorys();
		for(Inventory i : inventorys) {
			out.print(i.shipping);
			String trClass = "";
	...
	<%	
		...
		if(updatedId != null && updatedIdfromForm == null) {
			...
			Inventory i = inventoryDAO.getInventory(id);
	%>
			<hr>
			<h3> 修改存貨 </h3>
			<form class="form-inline" action="InventoryManagement.jsp" method="POST">
